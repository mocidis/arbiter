    ...
    while(1) {
        ...
        if (udata->o_head != NULL){
            DL_FOREACH_SAFE(udata->o_head, o_node, o_temp) {
                ...
                strncpy(areq.oiuc_gb.desc, o_node->desc, sizeof(o_node->desc));
        ...
        if (udata->r_head != NULL){
            DL_FOREACH_SAFE(udata->r_head, r_node, r_temp) {
                ...
                strncpy(areq.oiuc_gb.desc, r_node->desc, sizeof(r_node->desc));
                strncpy(areq.oiuc_gb.conn_str, r_node->ip_addr, sizeof(r_node->ip_addr));
                ...
                areq.oiuc_gb.radio_port = r_node->port;
